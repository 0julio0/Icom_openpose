...
def prediction(img3,start_time2):
    # print("img3", np.array(img3).shape)
    # img3= np.array(img3)
    try:
        y_pred3 = model.predict(img3)
    ...
    fps =  int(1.0 / (time.time() - start_time2))
    print("prediction ",fps, np.array(img3).shape)
    # print(fps)
    # print(classes[int(y_pred)], fps, c)
    # print(texto,"   ", int(fps), c)
    # return y_pred3
...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        # frameviewTotal_array=[]
        # p = multiprocessing.Process(target=prediction, args=(frameviewTotal_array,))
        # p.start()
            ...
            try:
                ...
                if len(frameviewTotal)==seq_len:
                    ...
                    if len(frameviewTotal)==seq_len:
                        ...
                        p = multiprocessing.Process(target=prediction, args=(frameviewTotal_array,start_time2,))
                        p.start()

                        print("normal    ",int(fps), frameviewTotal_array.shape)


                        # print(frameviewTotal_array.shape)
                        # y_pred = prediction(frameviewTotal_array,start_time2 )
