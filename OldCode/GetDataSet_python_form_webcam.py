...
while(True):
    ...
    file = open('output_json_icom/'+palavra+'_'+str(render_threshold)+'_'+stamp+".json","w") 
    ...
    while ((time.time() - start_recording_timer)>=2 and (time.time() - start_recording_timer)<5.5) and pp3<30:
        ...
        vid = sorted(glob.glob('output_json_icom/'+palavra+'_'+str(render_threshold)+'_'+stamp+".json"))
        ...
        imgList = np.array(img_gray).reshape(-1,frame_w2)
        imgFinal = imgList.tolist()
        json_str = json.dumps({"class": palavra,"frame":imgFinal})
        file.write(json_str+ '\n')
        ...
        img_gray = np.zeros((640,480,1), np.uint8) 
        k = cv2.waitKey(1)
